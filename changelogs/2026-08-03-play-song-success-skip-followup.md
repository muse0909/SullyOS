...
## 后续 fix (commit `ec2aa81`)
- 暮色 01:10 实测：放歌失败时 LLM 会撒谎说"放好了"——只发 `tool` 消息 + "放歌失败：xxx" 不够，LLM 仍然脑补"我放了用户没听到"
- 修法：跟生图失败同款，followup 多追加一条 `system` 消息明确指令"不要声称已放歌成功"
- 保留 `tool` 消息是 OpenAI 协议要求（不回传 tool_call_id LLM 会困惑可能再调一次）
- 暮色给的提示词版本："请你不要声称已经放歌成功。请自然地告诉用户：（原因）导致没有播放成功，问问用户要不要换首歌名或歌手名重试，或者自然的回应即可。"

## 待查
- **playSongFromChar 误返 null bug**（暮色 01:10 反馈）：之前放过能搜到的歌（"路过人间 郁可唯"）现在被标"没找到合适的版本"
- 不是 prompt 问题，是 `playSongFromChar` 内部匹配/搜歌逻辑可能有问题（30 分钟内放过的歌不重复？版权限制？匹配规则严格？）
- 不在今天范围，单独排查
