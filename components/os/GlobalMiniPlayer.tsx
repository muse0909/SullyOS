 ...
 * - 展开态右上角"≡ 队列"按钮（暮色 21:51 提的"播放列表功能"）—— 弹半屏浮层显示当前 queue，
 *   可点跳播 / 删除单首。
 *
    ...
    >
      ...
      >
        ...
        <div className="flex items-center gap-0.5 shrink-0">
          ...
          {/* 暮色 2026-08-01：队列按钮（"≡ 队列"）—— 弹半屏浮层显示 queue */}
          <button
            onClick={(e) => { e.stopPropagation(); setShowQueue(true); }}
            onPointerDown={(e) => e.stopPropagation()}
            className="p-1.5 rounded-full text-white/80 active:scale-95 transition-transform relative"
            aria-label="查看播放队列"
          ...
          >
            ...
            {queue.length > 1 && (
              <span
                className="absolute -top-0.5 -right-0.5 min-w-[14px] h-[14px] px-1 rounded-full text-[8px] font-medium flex items-center justify-center"
