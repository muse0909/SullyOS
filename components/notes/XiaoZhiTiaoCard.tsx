...
// 暮色原话："小纸条完全脱离小小窝 app" — 独立组件 / 独立视觉同 NotebookCard
...
// 暮色 2026-07-22：自定义样式（写入时随机选图当背景）+ 文字居中 + 避盖图

import React from 'react';
import { XiaoZhiTiao } from '../../types';

const TYPE_LABELS: Record<XiaoZhiTiao['type'], string> = {
    thought: '感想',
    doodle: '涂鸦',
    search: '搜索',
    lyric: '歌词',
...
interface TypeStyle {
    ...
    badgeBg: string;
    badgeText: string;
    patternCss?: React.CSSProperties;
}

const TYPE_STYLE: Record<XiaoZhiTiao['type'], TypeStyle> = {
    thought: {
        bg: '#dbeafe',
        ink: '#1e3a8a',
        badgeBg: '#1e3a8a',
...
// 5 种装饰物（圆钉/折角/钉子/回形针/胶带）
const StickerDecoration: React.FC<{ type: XiaoZhiTiao['type'] }> = ({ type }) => {
    switch (type) {
        case 'thought':
            return (
