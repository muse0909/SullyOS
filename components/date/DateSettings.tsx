...
const DateSettings: React.FC<DateSettingsProps> = ({ char, onBack }) => {
    ...
    // 暮色 2026-08-04 反馈：快捷键管理从这里移到见面 app 单独弹窗（齿轮按钮触发）
    //   DateSettings 不再管理快捷键 — 相关 state/handler/modal 全部删除
...
{/* ===== 长文模式主题 ===== */}
{/* 暮色 2026-08-04 反馈：「快捷键」section 从这里移走 — 改成见面 app 输入框上方齿轮按钮触发弹窗 */}
    ...
    >
      {char.dateBackground ? (
        ...
        <span className="text-slate-400 text-xs">+ 上传背景图</span>
            ...
            {/* 暮色 2026-08-04：快捷键弹窗已从 DateSettings 移到 DateSession（齿轮按钮触发），这里不再有 modal */}
