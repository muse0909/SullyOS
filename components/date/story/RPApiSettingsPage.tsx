            ...
            <div className="relative z-10 flex-1 overflow-y-auto no-scrollbar px-5 py-4 space-y-4">
                ...
                <section className="rounded-2xl p-3" style={{ border: '1px dashed rgba(167,139,250,0.4)', background: 'rgba(255,255,255,0.45)' }}>
                    ...
                    >
                        ...
                        <span className="text-[14px] font-bold tracking-wider" style={{ color: '#4a3a6a' }}>API 设置</span>
                    ...
                    {apiSectionOpen && (
                        <div className="mt-3 space-y-3">
                                ...
                                {configs.map(cfg => (
                                    <div key={cfg.id} className="flex items-center gap-1">
                                        ...
                                        <button onClick={() => handleTest(cfg)} disabled={testingId === cfg.id}
                                                className="w-7 h-7 rounded-lg flex items-center justify-center disabled:opacity-50 active:scale-90"
                            ...
                            {configs.length === 0 && (
                                <div className="text-center text-[10px]" style={{ color: 'rgba(150,120,190,0.55)' }}>
                                    点胶囊 = 设为 RP 默认 API;自建 RP 配可测通/编辑/删除
                                </div>
                            )}
                            <button onClick={handleNew} className="w-full py-2 rounded-xl text-[11px] font-bold flex items-center justify-center gap-1.5 active:scale-95 transition-all"
                ...
                {/* ─── Section 2:全局默认配置(暮色 8-26 17:30:去掉虚线外框,跟其他内容视觉一致) ─── */}
                <section className="py-2">
                    ...
                    {draftDefaults && (
                        <div className="space-y-4">
                            ...
                            {/* 暮色 9-21 第三轮:默认注入主聊天最近 50 条聊天记录 — 全局默认开关 */}
                            <Field label="注入聊天记录" hint="建完剧场进 Session 时,自动从主聊天取最近 50 条拼到开场 context(仅这一次,不让角色断档)。关掉就开剧场前不告诉角色你俩聊过什么。">
                                <button
                                    type="button"
                                    onClick={() => setDraftDefaults({ ...draftDefaults, injectChatHistory: !(draftDefaults.injectChatHistory !== false) })}
                                    className="mt-1.5 w-full flex items-center gap-2.5 rounded-xl px-3 py-2 active:scale-[0.98] transition-all text-left"
                                    style={{
                                        background: draftDefaults.injectChatHistory !== false ? 'rgba(167,139,250,0.1)' : 'rgba(255,255,255,0.55)',
                                        border: draftDefaults.injectChatHistory !== false ? '1.5px solid #a78bfa' : '1px solid rgba(170,140,210,0.3)',
                                    }}
                                >
                                    <div
                                        className="w-4 h-4 rounded flex-shrink-0 flex items-center justify-center"
                                        style={{
                                            background: draftDefaults.injectChatHistory !== false ? '#7c3aed' : 'white',
                                            border: draftDefaults.injectChatHistory !== false ? 'none' : '1.5px solid rgba(150,120,190,0.4)',
                                        }}
                                    >
                                        {draftDefaults.injectChatHistory !== false && <Check size={11} weight="bold" style={{ color: 'white' }} />}
                                    </div>
                                    <div className="flex-1">
                                        <div className="text-[12px] font-bold" style={{ color: '#4a3a6a' }}>
                                            {draftDefaults.injectChatHistory !== false ? '新剧场默认注入聊天记录' : '不注入(角色不知道开剧场前聊过)'}
                                        </div>
                                    </div>
                                </button>
                            </Field>
...
/* 暮色 8-26 17:30:API 胶囊预设 — 像系统设置里 API 预设的样式,但紫色调
   选中态:紫渐变背景 + 紫边框
   非选中态:白底淡紫字 + 淡紫边框
   只显示名字 */
const ApiPill: React.FC<{
    selected: boolean;
    onClick: () => void;
    label: string;
    ...
    return (
        <button
            onClick={onClick}
            className="rounded-full px-3 py-1.5 text-[12px] font-bold active:scale-95 transition-all"
            style={{
                background: selected
                    ? 'linear-gradient(135deg,rgba(167,139,250,0.22),rgba(124,58,237,0.12))'
