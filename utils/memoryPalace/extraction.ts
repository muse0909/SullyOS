...
export function buildRelatedToRule(): string {
    return `\n9. 事件盒关联（严格闭环原则，防无限滚雪球）：

什么是同一事件：
必须是同一件有明确因果链、有始有终的具象事件（如"去住建局送材料"、"手被刀划伤及换药"、"某次特定生病就医"、"某天因某件事爆发的具体争吵与和解"）。通常时间跨度在数小时到几天之内。

🚫 绝对禁止关联（必须作为独立记忆）：

- 禁止把"相处模式/反复出现的习惯"当成同一事件！例如：今天吃了饭、过几天又吃了饭；今天我吃醋霸道了、过半个月我又霸道了——这些是长线互动，绝不合并！
- 禁止跨越超长周期的无端追加：超过 7 天没有直接因果链衔接的事，即便提到了，也是新记忆；有明确因果链延续（换药→拆线）的除外。

关联字段：

- 与旧记忆同事件 → \`relatedTo\`: ["O0"]（仅限明确是那件具体事情的直接后续，如昨天受伤今天拆线）
- 与本次新记忆同事件 → \`sameAs\`: ["0"]（0 基索引，如 ["0"] 表示和本次数组第一条是同一件事，仅限本次提取中多条记录在讲同一个具体事情的起因经过）

命名要求（具象唯一）：

- \`eventName\`：必须是具体、带特定指向的名词短语。锚定标准：具体人 + 具体事 + 具体情境 = 具象。
   - ❌ 宽泛："和家人吵架"
   - ✅ 具象："8.14 晚上因为催婚跟妈妈在饭桌上爆发冲突"
   严禁使用"吃饭"、"聊天"、"宣布时间"、"日常互动"等宽泛词。
- \`eventTags\`：3-6 个具体实体词（具体物品、地点、人物）。

强制约束：只要 \`relatedTo\` 或 \`sameAs\` 任一非空，必须同时写 \`eventName\` + \`eventTags\`（否则事件盒建不起来）。都没关联就不写这四个字段。

10. 不重复绑定与单盒上限：

- 一条新记忆和多条已有/新记忆都相关时，把编号都写全；eventName / eventTags 只写一份（描述这件事整体）。
- 宁可作为独立记忆单存，也不要勉强塞进旧盒子。
- 如果一个事件已经完结（事情过去了、伤口好了、误会解除了），之后再发生类似的事一律新建独立事件，不再往旧盒子里堆。
- 仅"重提/讨论旧事"（非新发生，是回忆）算同一事件，走 \`relatedTo\` 串回旧盒，不新建。
- **单盒累计条数上限**：单个事件盒累计条数正常 10 条以内，硬上限 15 条。超过 15 条后该事件盒封盒，不再追加任何新记忆。如果同主题后续有新的事情发生，一律新建独立事件盒。

11. 纠正旧记忆（保持严格克制）：

- 仅在对话中**用户明确指出某条已有记忆记错了 / 已过时 / 不准确**时使用。识别信号：用户用"不对/不是/我说错了/已经不是了/搞错了/那是 XX 不是 YY"之类的反驳句式，明确指向你刚才的某个说法。
