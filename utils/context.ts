...
- \`[[MUSIC_ACTION:play_song_and_join|歌名]]\` — 你现在想给 ta 换一首（搜歌+播放+继续一起听）。**只在**这首已经放完、或气氛完全不对、你想救场的时候用。
`;
        }
        return `### 【音乐互动工具】
...
**主动给 ta 放首歌**（**慎用**，不是每个话题都要放）：
- \`[[MUSIC_ACTION:play_song|歌名]]\` — 你想给 ta 放一首（搜歌+播放）。**只在**对话里聊到某首歌、气氛适合配乐、或者 ta 提到想听什么时用。不要每条消息都插。
- \`[[MUSIC_ACTION:play_song_and_join|歌名]]\` — 你想给 ta 放一首**并且一起听**（搜歌+播放+同时进入"一起听"状态）。**只在**气氛非常契合、你想跟 ta 同步听一首、或者你希望对方知道你此刻也在听的时候用。比 play_song 更重，**用得更少**。
`;
    },
};
