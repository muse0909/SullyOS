...
>
    ...
    复制 ({selectedCount})
        ...
        <button
            ...
            className={`flex-1 py-3 bg-red-500 text-white font-bold rounded-xl shadow-lg active:scale-95 transition-transform flex items-center justify-center gap-2`}
        >
                ...
                <div className="p-2.5 px-2.5 flex gap-1.5 items-end">
                    ...
                    <div className={`flex-1 min-w-0 flex items-center transition-all ${useIOSStandaloneInputFix ? 'overflow-visible' : 'overflow-hidden'} ${inputWrapClass} ${isPixelStyle ? 'focus-within:bg-[#fff7ed]' : isDiscordStyle ? 'focus-within:bg-slate-800 focus-within:border-white/20' : 'border border-transparent focus-within:bg-white focus-within:border-primary/30'}`}>
                        <textarea
                            ...
                            className={`flex-1 min-w-0 bg-transparent px-1.5 py-3 ${useIOSStandaloneInputFix ? 'text-[16px]' : 'text-[15px]'} resize-none max-h-40 no-scrollbar ${isDiscordStyle ? 'text-white placeholder:text-slate-500' : isPixelStyle ? 'text-[#6a4c35] placeholder:text-[#9b8677]' : ''}`}
                            placeholder="Message..."
