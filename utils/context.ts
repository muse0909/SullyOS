...
export const ContextBuilder = {
  ...
  收进来的歌会被打上"从对方那里听到"的标签 —— 以后你单独听到这首时，会自然想起 ta。`;
        // 暮色 2026-08-02 00:25 江澈转达诉求 5：直接输出动作令牌 / 不要先宣布
        const playSongGuide = `**主动给 ta 放首歌**（**慎用**，不是每个话题都要放）：
  - \`[[MUSIC_ACTION:play_song|歌名]]\` — 你想给 ta 放一首（搜歌+播放）。**只在**对话里聊到某首歌、气氛适合配乐、或者 ta 提到想听什么时用。
  - \`[[MUSIC_ACTION:play_song_and_join|歌名]]\` — 你想给 ta 放一首**并且一起听**（搜歌+播放+同时进入"一起听"状态）。比 play_song 更重，**用得更少**。
  **使用规则**：决定使用 \`play_song\` / \`play_song_and_join\` 时，**直接输出动作令牌**（塞在文本任意位置），不要先宣布"现在执行指令"，不要让用户负责确认前端是否生效。只有收到失败回执时再自然说明（"想放这首但没找到合适的版本"）。`;
        if (isListeningTogether) {
            return `### 【音乐互动工具】
你们正在一起听同一首歌。
