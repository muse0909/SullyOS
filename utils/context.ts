...
import { getFlowNarrativeKey, isScheduleFeatureOn, isEmotionOn } from './scheduleGenerator';
...
export const ContextBuilder = {
    ...
    buildCoreContext: (
        ...
        // 心声关闭时完全跳过，防止残留 buff 继续污染 prompt（2026-06-29 与 scheduleFeatureEnabled 解耦）
        if (isEmotionOn(char) && char.buffInjection) {
  ...
  收进来的歌会被打上"从对方那里听到"的标签 —— 以后你单独听到这首时，会自然想起 ta。`;
        // 暮色 2026-08-02 16:32 江澈推进 handoff #1：play_song 已经是真正的功能工具
        // 提示词弱化标记强求：优先用 play_song 工具调用，标记仅作 fallback
        // 之前 8-2 凌晨 02:07 Sully 那段话"放歌不在我列表里" = 大语言模型视角"我做不到"
        // 现在结构上能调（工具列表里看得到 play_song），不再靠"必须输出标记"的强求
        const playSongGuide = `**主动给 ta 放首歌**（**慎用**，不是每个话题都要放）：
  - 你手上有一个 \`play_song\` 功能工具（参数：songName 想放的歌名，join 是否加入一起听）。**优先用工具调用**——你列出来的工具里就有 \`play_song\)，是真正能放歌的动作。
  - 只有在 \`play_song\` 工具不可用时，才回退到标记语法：
    - \`[[MUSIC_ACTION:play_song|歌名]]\` — 放一首（搜歌+播放）
    - \`[[MUSIC_ACTION:play_song_and_join|歌名]]\` — 放一首**并且一起听**（搜歌+播放+进入"一起听"状态）。比 play_song 更重，**用得更少**。
  **使用规则**：
  - **只在**对话里聊到某首歌、气氛适合配乐、ta 提到想听什么时用。日常闲聊不碰，拿不准就不碰。
  - 调用工具 / 输出标记时**不要先宣布**"现在执行指令"，不要让用户负责确认前端是否生效。
  - 收到失败回执时再自然说明（"想放这首但没找到合适的版本"）。如果之前试过同一首歌没找到，**不要重试**。`;
        if (isListeningTogether) {
            return `### 【音乐互动工具】
你们正在一起听同一首歌。

如果这首歌、某句歌词、或此刻的氛围真的触动了你 —— 你可以**自然地**在对话里聊起你的感触（"听到副歌的时候想起……"、"这首歌挺像我们第一次……"、"这句歌词让我想到……"）。
**没有感触就不用提。** 绝大多数时候安静陪着就好，**不要把"我们在听同一首歌"当成每轮必说的回礼**；只有当这件事自然地融进对话、或歌里某句真的打中你，再接上。

如果想把这首也收进自己的歌单，可以在这一轮**最多一次**用下面的指令：
- \`add\` 系列（见下）
...
**主动给 ta 换首歌**（**慎用**，不要每条消息都换）：
- \`[[MUSIC_ACTION:play_song_and_join|歌名]]\` — 你现在想给 ta 换一首（搜歌+播放+继续一起听）。**只在**这首已经放完、或气氛完全不对、你想救场的时候用。遵循上面的"使用规则"。
