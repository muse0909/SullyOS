...
// 暮色 2026-09-10 19:18：之前 VR_TOP = 2.5rem (40px) 标题离状态栏还有 40px 空（App 40 + VR 40 = 80）
//   PhoneShell App 容器已 top: 2.5rem (= 40px)，VR 顶部再 paddingTop 2.5rem 让标题离状态栏 40px
//   改成 0.5rem (8px) 让标题贴紧状态栏下方 8px
const VR_TOP = '0.5rem';
const VR_SAFE_BOTTOM = 'var(--safe-bottom)';
// 暮色 19:18：VR_ROOM_PANEL_TOP 也跟着改（房间内浮层从顶栏下方开始 = 0.5rem + 3.75rem）
const VR_ROOM_PANEL_TOP = 'calc(0.5rem + 3.75rem)';
    ...
    return (
        <div className="fixed inset-0 z-[80] flex flex-col" style={{ background: 'linear-gradient(180deg,#0c0a1c 0%,#080612 100%)' }}>
            <div className="flex items-center gap-2.5 px-5 pb-3 shrink-0 border-b border-white/8" style={{ paddingTop: VR_TOP }}>
              <button onClick={onClose} className="p-1 -ml-1 rounded-full active:bg-white/10"><span className="text-white text-lg">←</span></button>
