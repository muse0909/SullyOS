...
但有两条铁律：
1. **别替 ta 写台词或反应。** 你只管写"你自己"对着 ta 的形象做了什么、说了什么；至于 ${uname} 怎么回应、要不要理你、是什么表情，全部留白，交给 ta 本人——绝不能替 ta 编台词、编动作、编心理。
2. **别把这当成 ta 本人真的在线。** 这只是 ta 挂在虚拟空间里的一个化身状态（类似游戏挂机 / AFK），并不代表 ${uname} 此刻真守在游戏里——ta 很可能早已离开屏幕、在现实里忙别的或休息。所以别据此认定"ta 正盯着你""ta 现实里也在干这件事"，也别把它当成 ta 在跟你说话。你和 ta 的真实关系、近况一律以你们的聊天记录为准；这条只是彼方这个虚拟空间里的一个在场提示而已。\n`;
            }
        }
