...
1. **第 8 格卡片「按日期时间线」**（窗台右边空位，浅蓝 `#0ea5e9`）
   ...
   - 点进去是 **3 级导航视图**（仿神经连接 `MemoryArchivist`）：
     - **Root**：年份卡片网格（3 列，大数字 + "年" + 短分割线 + "X 条记忆" + 右上角小锚点）
     - **Year**：月份卡片网格（3 列，同款卡片）
     - **Month**：**4 级分组**「上旬 (1-10) / 中旬 (11-20) / 下旬 (21-31)」→ 日分组（左侧浅蓝竖线 + 「7月13日 周日」格式）→ 记忆列表
   - 顶部 bar **sticky 吸顶**：滚动时返回按钮 + 标题 + 面包屑 + 计数一直可见（半透明白底 + 8px 模糊）
   - 顶部面包屑：`档案 / 2026 / 7月`，可点击回上一级
   - 导航状态保持：进 memory 详情再返回会回到 month（不再是 year），用 `tlInitialized` 标记只在首次/重入时重置导航
